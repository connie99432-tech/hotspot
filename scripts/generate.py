...
def build_prompt():
    ...
    return (diliao+"\n\n=== 以上是部门底料，据此筛选/路由/按第八节格式与铁律写 ===\n\n"
        ...
        +"全网热榜：\n"+hot_lists()+"\n\n"
        +"营销垂媒/行业(广告门+36氪)：\n"+rss_titles()+"\n\n"
        +"写一份【营销热点日报 · "+today+"】：首行 `# 营销热点日报 · "+today+"`；二行'今天最值得关注的：…'不带括号；6板块每板块≤2条精选；每条'【看得懂标题】：一两句。@花名 关注，可考虑…'，@直接跟花名不加'建议'不带部门名；标题正文说人话不用黑话、绝不写'（窗口X）'；@后建议要礼貌简短、商量口吻，不批评不甩锅不教训（不写'别再…''别只…''别被…盖过'这类）；竞对板块务必含喂料线索；结尾'以上各业务侧可参考&评估跟进~'另起一行'内容由AI助手整理发布，有问题或建议请随时联系huke1。'。遇重大灾害/伤亡事故(山体崩塌、地震、重大事故等)只客观简述、当作一条热点让大家知道即可，**不@任何对接人、不给营销建议**(尺度难控、不消费苦难)；只输出播报正文。")
